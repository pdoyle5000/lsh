...
class LSH:
    def __init__(
        ...
        num_hashes: int,
        num_tables: int,
        planes: Optional[List[np.ndarray]] = None,
    ):
        ...
        if planes is None:
            self.planes = [
                np.random.randn(self.hash_byte_len, self.num_hashes)
    ...
    def query(self, vector: List, k: int = None, dist_func=euclidean_dist):
        ...
        for loc, htable in enumerate(self.hash_tables):
            this_hash = self.hash(self.planes[loc], vector)
            contestants.extend(htable.get(this_hash))  # type: ignore
        candidates = [
            (contestent, dist_func(vector, contestent[0])) for contestent in contestants
        ]
        candidates.sort(key=lambda x: x[1])
        candidate_data = list(
            OrderedDict.fromkeys([candidate[0][1] for candidate in candidates])
        )
